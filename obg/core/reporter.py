...
def generate_error_log(device: str, model: str, error: str, steps: list) -> str:
    report_dir = reports_dir()
    safe_model = "".join(c if c.isalnum() or c in "-_" else "_" for c in model)
    from datetime import datetime
    ts = datetime.now().strftime("%Y%m%d-%H%M%S")
    filename = f"OldButGold-ERROR-{ts}-{safe_model}.md"
    path = os.path.join(report_dir, filename)
    lines = [
        "# OldButGold — Pipeline Error Log",
        "",
        f"**Device:** {device}",
        f"**Model:** {model}",
    ...
    for s in steps:
        icon = "OK" if s.status.value == "ok" else ("FAILED" if s.status.value == "failed" else s.status.value.upper())
