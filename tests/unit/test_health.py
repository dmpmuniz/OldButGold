...
def test_read_smart_parses_thresholds_and_when_failed():
    output = """smartctl 7.1 2019-10-24 r5009
...
Vendor Specific SMART Attributes with Thresholds:
ID# ATTRIBUTE_NAME          FLAG     VALUE WORST THRESH TYPE      UPDATED  WHEN_FAILED RAW_VALUE
  5 Reallocated_Sector_Ct   0x0033   073   073   036    Pre-fail  Always       -       140
  9 Power_On_Hours          0x0032   097   097   000    Old_age   Always       -       12345
 187 Reported_Uncorrectable 0x0032   100   100   000    Old_age   Always       -       0
 197 Current_Pending_Sector  0x0012   060   060   000    Old_age   Always  FAILING_NOW       12
 198 Offline_Uncorrectable   0x0030   100   100   000    Old_age   Offline      -       0
 199 UDMA_CRC_Error_Count    0x0032   200   200   000    Old_age   Always       -       0
"""
    mock_result = MagicMock()
    ...
    assert sd is not None
    assert sd.reallocated_sectors == 140
    attr5 = sd.attributes[5]
    assert (attr5.value, attr5.worst, attr5.thresh, attr5.when_failed, attr5.raw) == (73, 73, 36, "-", 140)
    attr197 = sd.attributes[197]
    assert attr197.when_failed == "FAILING_NOW"
    assert attr197.raw == 12
    attr198 = sd.attributes[198]
    assert attr198.when_failed == "-"


def test_read_smart_crc_errors_uses_max_of_187_and_199():
    output = """smartctl 7.1
...
ID# ATTRIBUTE_NAME          FLAG     VALUE WORST THRESH TYPE      UPDATED  WHEN_FAILED RAW_VALUE
 ...
 187 Reported_Uncorrectable 0x0032   083   083   000    Old_age   Always       -       17
 197 Current_Pending_Sector  0x0012   100   100   000    Old_age   Always       -       0
 198 Offline_Uncorrectable   0x0030   100   100   000    Old_age   Offline      -       0
"""
    mock_result = MagicMock()
